...
use risc0_zkvm::host::server::exec::ExecutorImpl;
...
fn execute(){
    ...
    let env = ExecutorEnv::builder()
        ...
        .build()
        .unwrap();
    // pub const TEST_ELF: &[u8] = include_bytes!("/mnt/sdd1/sbw/zkvm/poc/load_bug.elf");
    // let mut exec = ExecutorImpl::from_elf(env, TEST_ELF).unwrap();
    let mut exec = ExecutorImpl::from_elf(env, HELLO_GUEST_ELF).unwrap();
    let err: anyhow::Error = exec.run().err().unwrap();
    println!("error: {}",err);
}
